...
resolve_python_cmd() {
  _clv2_try_python() {
    ...
    "$cmd" -c "import sys" >/dev/null 2>&1 || return 1
    printf '%s\n' "$cmd"
    return 0
  }

  if [ -n "${CLV2_PYTHON_CMD:-}" ]; then
    _clv2_try_python "$CLV2_PYTHON_CMD" && return 0
  fi

  _clv2_try_python python && return 0
  _clv2_try_python python3 && return 0
...
if [ -n "$STDIN_CWD" ] && [ -d "$STDIN_CWD" ]; then
  _GIT_COMMON_DIR=$(git -C "$STDIN_CWD" rev-parse --path-format=absolute --git-common-dir 2>/dev/null || true)
  _GIT_ROOT=""
  case "$_GIT_COMMON_DIR" in
    */.git) _GIT_ROOT="${_GIT_COMMON_DIR%/.git}" ;;
  esac
  if [ -z "$_GIT_ROOT" ]; then
