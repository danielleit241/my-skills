  ...
  for (const [dirIndex, dir] of searchDirs.entries()) {
    ...
    for (const match of matches) {
      ...
      if (
        ...
        (current.mtime === existing.mtime &&
          current.dirIndex < existing.dirIndex)
      ) {
...
function pruneExpiredSessions(searchDirs, retentionDays) {
  const uniqueDirs = Array.from(
    new Set(
      searchDirs.filter((dir) => typeof dir === "string" && dir.length > 0),
    ),
  );
  let removed = 0;
...
function selectMatchingSession(sessions, cwd, currentProject) {
  ...
  for (const session of sessions) {
    ...
    if (!projectMatch && currentProject) {
      ...
      const sessionProject = projectFieldMatch
        ? projectFieldMatch[1].trim()
        : "";
  ...
  if (projectMatch) {
    return {
      session: projectMatch,
      content: projectMatchContent,
      matchReason: "project",
    };
  }
...
function extractInstinctAction(content) {
  const actionMatch = String(content || "").match(
    /## Action\s*\n+([\s\S]+?)(?:\n## |\n---|$)/,
  );
  const actionBlock = (
    actionMatch ? actionMatch[1] : String(content || "")
  ).trim();
...
function summarizeActiveInstincts(observerContext) {
  ...
  const globalDirs = [
    ...
    {
      dir: path.join(homunculusDir, "instincts", "inherited"),
      scope: "global",
    },
  ];
  ...
  const scopedInstincts = [
    ...projectDirs.flatMap(({ dir, scope }) =>
      readInstinctsFromDir(dir, scope),
    ),
  ...
  for (const instinct of scopedInstincts) {
    if (!instinct.id || instinct.confidence < INSTINCT_CONFIDENCE_THRESHOLD)
      continue;
  ...
  log(
    `[SessionStart] Injecting ${ranked.length} instinct(s) into session context`,
  );
...
async function main() {
  ...
  if (recentSessions.length > 0) {
    ...
    if (result) {
      log(
        `[SessionStart] Selected: ${result.session.path} (match: ${result.matchReason})`,
      );
